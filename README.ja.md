...
Markdownヘッダーは `0` 行目、データ行は `1` 行目から表示します。未選択の行または列の見出しをドラッグすると、その軸だけの連続範囲を選択します。選択済みの見出しをもう一度ドラッグすると、選択ブロックを移動できます。並べ替え中は移動対象が半透明になり、ドラッグ中のプレビューと挿入予定位置のラインが表示されます。見出しだけでなく対応するセル上でもドロップ位置を指定でき、セルの前半・後半で挿入前後を選べます。
