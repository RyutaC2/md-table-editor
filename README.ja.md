...
ツールバーは指定された操作順ですべて左寄せにします。1500px以上では名称を含む1段、1100～1499pxでは同じ順序のアイコンのみ1段、700～1099pxでは上段を貼り付け系・Undo系・行系・ズーム、下段を配置系・インポート系・列系とするアイコンのみ2段になります。699px以下ではインポート・エクスポート・ズームを幅を確保した左端の複合メニューへ格納し、上段を貼り付け系・行系、下段を複合メニュー・配置系・Undo系・列系とする2段表示になります。どの幅でもツールバーにスクロールバーは表示せず、すべての操作を利用できます。複数行・列の追加は選択数と同じ数を一括追加します。列見出しの `more_horiz` メニューでは列ごとの配置とソートを実行でき、操作後またはメニュー外のクリックで閉じます。メニューは左右の表示領域から見切れないよう自動調整され、現在の配置はチェックで確認できます。配置はMarkdownソースとGUIのヘッダー・本文・編集中セルへ即時反映されます。
