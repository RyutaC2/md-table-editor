...
ツールバーは表示幅に応じて、1500px以上ではラベル付き1段、800～1499pxではラベル付き2段、799px以下ではアイコンのみ1段へ切り替わります。2段表示では行操作の直下に列操作を配置し、ツールバー内にはスクロールバーを表示しません。操作ジャンル間には区切りと余白を設けますが、Undo・Redo・`横幅を整える` は区切らず連続して配置しています。コピー、切り取り、選択セルの内容削除もツールバーから実行できます。列見出しの `more_horiz` メニューからは配置とソートを実行でき、操作後またはメニュー外のクリックで閉じます。メニューは左右の表示領域から見切れないよう自動調整され、現在の配置はメニュー内のチェックで確認できます。左・中央・右の配置は、MarkdownソースとGUIのヘッダー・本文・編集中セルへ即時反映されます。`指定なし` の列はGUI上では左寄せで表示します。
