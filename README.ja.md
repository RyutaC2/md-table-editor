...
ツールバーは指定された操作順ですべて左寄せにします。1500px以上では1段となり、行列操作だけ名称を表示します。1050～1499pxでは各操作を直接選べる2段表示、1049px以下ではインポート・エクスポート・ズームを左端メニューへ、配置・行・列操作を個別メニューへ格納した2段表示になります。どの幅でもツールバーにスクロールバーは表示せず、貼り付け、切り取り、コピー、内容削除、Undo、Redo、ファイル操作、ズーム、全列配置を利用できます。複数行・列の追加は選択数と同じ数を一括追加します。列見出しの `more_horiz` メニューでは列ごとの配置とソートを実行でき、操作後またはメニュー外のクリックで閉じます。メニューは左右の表示領域から見切れないよう自動調整され、現在の配置はチェックで確認できます。配置はMarkdownソースとGUIのヘッダー・本文・編集中セルへ即時反映されます。
