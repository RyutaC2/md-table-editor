...
The left-aligned toolbar follows the requested action order. At 1500px or wider it uses one row and shows names for row and column operations. From 1050px to 1499px it uses two rows with direct action buttons. At 1049px or narrower, file import/export and zoom move into the far-left menu, while alignment, row, and column actions use separate menus. No layout adds a toolbar scrollbar. Paste, cut, copy, clear, Undo, Redo, file actions, zoom, and whole-table alignment remain available at every width. Row or column insertion repeats by the number of selected rows or columns. The `more_horiz` menu in each column heading continues to provide per-column alignment and sorting, closes after an action or an outside click, stays within the visible grid area, and marks the current alignment. Alignment is reflected immediately in the Markdown source and in header, body, and active editor cells.
