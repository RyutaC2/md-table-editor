...
ツールバーは2段構成で、履歴・行操作を上段、列操作・表示操作を下段へグループ化しています。各操作には方向を区別できるアイコンとラベルを表示し、ツールバー内にはスクロールバーを表示しません。列見出しのメニューからは配置とソートを実行でき、操作後はメニューが閉じます。現在の配置は列見出しのアイコンへ反映されます。
