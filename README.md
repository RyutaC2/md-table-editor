...
ツールバーは表示幅に応じて、1500px以上ではラベル付き1段、680～1499pxではラベル付き2段、679px以下ではアイコンのみ1段へ切り替わります。2段表示では行操作の直下に列操作を配置し、ツールバー内にはスクロールバーを表示しません。コピー、切り取り、選択セルの内容削除もツールバーから実行できます。列見出しのメニューからは配置とソートを実行でき、操作後はメニューが閉じます。現在の配置は列見出しのアイコンへ反映されます。
...
UIアイコンにはGoogle Material Symbols由来のSVGをローカル同梱しています。アイコンの形状はVS Codeテーマによって変化せず、色はVS Codeのテーマ色へ追従します。第三者ライセンスは [THIRD_PARTY_NOTICES.md](./THIRD_PARTY_NOTICES.md) に記載しています。
