...
The left-aligned toolbar follows the requested action order. At 1500px or wider it uses one labeled row; from 1100px to 1499px it keeps that order in one icon-only row. From 700px to 1099px it uses two icon-only rows grouped as clipboard/history/rows/zoom above alignment/files/columns. At 699px or narrower, import, export, and zoom move into a wider compound menu while the remaining direct controls use two rows. No layout adds a toolbar scrollbar. Paste, cut, copy, clear, Undo, Redo, file actions, zoom, and whole-table alignment remain available at every width. Row or column insertion repeats by the number of selected rows or columns. The `more_horiz` menu in each column heading continues to provide per-column alignment and sorting, closes after an action or an outside click, stays within the visible grid area, and marks the current alignment. Alignment is reflected immediately in the Markdown source and in header, body, and active editor cells.
